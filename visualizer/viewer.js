(() => {
  ...
  const partIds=Object.keys(D.parts).map(Number).sort((a,b)=>a-b),predPartIds=Object.keys(P.parts).map(Number).sort((a,b)=>a-b),maxPart=Math.max(0,...partIds,...predPartIds);
  ...
  slider.max=N-1;
  function selectionUI(){document.querySelectorAll('[data-part]').forEach(b=>b.setAttribute('aria-pressed',Number(b.dataset.part)===selection));const axisNote=P.axes.length?'Yellow: confidence-filtered predicted axis':'No joint axis passed the confidence threshold';detail.textContent=selection?`${mode===3?P.parts[selection]:D.parts[selection]} · ${flow?'CoTracker tracks shown':'Tracks hidden'} · Other parts dimmed`:(mode===3?`${Object.values(P.parts).join(' / ')} · ${axisNote} · Gray: unassigned`:'Click the object to select a part. Drag to orbit in RGBD / 4D.');}
  ...
  function layout(){const scale=Math.min(W/cw,H/ch);return{scale,x:(W-cw*scale)/2,y:(H-ch*scale)/2};}
  function project(x,y,z){const a=viewYaw*Math.min(1,morph),b=viewPitch*Math.min(1,morph);x-=C[0];y-=C[1];z-=C[2];const xx=x*Math.cos(a)+z*Math.sin(a),zz=-x*Math.sin(a)+z*Math.cos(a),yy=y*Math.cos(b)-zz*Math.sin(b);z=y*Math.sin(b)+zz*Math.cos(b)+C[2];x=xx+C[0];y=yy+C[1];const l=layout();return[l.x+(K.fx*x/z+K.cx-cx)*l.scale,l.y+(-K.fy*y/z+K.cy-cy)*l.scale,z];}
  function maskAt(x,y){const l=layout(),u=Math.floor((x-l.x)/l.scale),v=Math.floor((y-l.y)/l.scale);return u>=0&&u<cw&&v>=0&&v<ch?mask[v*cw+u]:0;}
  canvas.onpointerdown=e=>{if(!ready||drag)return;drag={id:e.pointerId,x:e.clientX,y:e.clientY,lx:e.clientX,ly:e.clientY,moved:false};canvas.setPointerCapture(e.pointerId);};
  ...
  function draw(now){if(!canvas.isConnected)return;const dt=Math.min((now-last)/1000||0,.05);last=now;requestAnimationFrame(draw);if(!ready)return;
    ...
    ctx.globalAlpha=(1-spatial)*dim[0];ctx.drawImage(imgs[frame],l.x,l.y,cw*l.scale,ch*l.scale);
    for(const part of partIds){ctx.globalAlpha=(1-spatial)*(dim[part]-dim[0]);if(ctx.globalAlpha>0)ctx.drawImage(surfaces[part-1],l.x,l.y,cw*l.scale,ch*l.scale);}
    ctx.globalAlpha=1;hits=[];
    if(spatial>.001){const raw=frames[frame].points,points=[];for(let i=0;i<raw.length;i+=8){const view=raw[i+7];if(view)continue;const p=project(raw[i],raw[i+1],raw[i+2]);if(p[2]<=.05)continue;const part=mode===3?P.labels[frame][i/8]:raw[i+6];const palette=[[113,124,137],[95,188,255],[238,154,91],[169,122,255],[255,210,94]],blend=Math.max(0,Math.min(1,morph-2)),color=palette[part]||palette[0];points.push([...p,part,...color.map((v,j)=>raw[i+3+j]*(1-blend)+v*blend),view]);}points.sort((a,b)=>b[2]-a[2]);
      for(const p of points){const alpha=spatial*(p[7]?fusion:1)*dim[p[3]];ctx.globalAlpha=alpha;const chosen=selection===p[3],r=chosen?Math.min(255,p[4]*.65+55):p[4],g=chosen?Math.min(255,p[5]*.65+90):p[5],b=chosen?Math.min(255,p[6]*.65+75):p[6];ctx.fillStyle=`rgb(${r},${g},${b})`;const size=l.scale*(3.2-fusion*1.5);ctx.fillRect(p[0]-size/2,p[1]-size/2,size,size);if(alpha>.06)hits.push(p);}}
    ctx.globalAlpha=1;
    if(selection&&flow){for(const tr of (mode===3?P.tracks:tracks)){if(tr.part!==selection||tr.view&&fusion<.3)continue;const current=tr.samples[frame];if(!current)continue;ctx.beginPath();let open=false;for(let j=Math.max(0,frame-12);j<=frame;j++){const s=tr.samples[j];if(!s){open=false;continue;}const p=project(s[0],s[1],s[2]);if(p[2]<.05){open=false;continue;}if(tr.view===0){const ux=l.x+(s[3]-cx)*l.scale,uy=l.y+(s[4]-cy)*l.scale;p[0]=ux*(1-spatial)+p[0]*spatial;p[1]=uy*(1-spatial)+p[1]*spatial;}open?ctx.lineTo(p[0],p[1]):ctx.moveTo(p[0],p[1]);open=true;}ctx.globalAlpha=tr.view?fusion:.85;ctx.strokeStyle=['#e8fff8','#78d9ff','#9bffd0','#c7a1ff'][selection]||'#ffe078';ctx.lineWidth=1.3;ctx.stroke();let p=project(...current.slice(0,3));if(tr.view===0){p[0]=(l.x+(current[3]-cx)*l.scale)*(1-spatial)+p[0]*spatial;p[1]=(l.y+(current[4]-cy)*l.scale)*(1-spatial)+p[1]*spatial;}ctx.fillStyle='#e8fff8';ctx.beginPath();ctx.arc(p[0],p[1],2,0,Math.PI*2);ctx.fill();}}
    ctx.globalAlpha=1;
    if(mode===3){for(const axis of P.axes){const ends=[-0.24,0.24].map(k=>project(...axis.pivot.map((v,i)=>v+k*axis.direction[i])));const pivot=project(...axis.pivot);ctx.strokeStyle='#ffe078';ctx.lineWidth=3;ctx.beginPath();ctx.moveTo(ends[0][0],ends[0][1]);ctx.lineTo(ends[1][0],ends[1][1]);ctx.stroke();ctx.fillStyle='#fff2b3';ctx.beginPath();ctx.arc(pivot[0],pivot[1],5,0,Math.PI*2);ctx.fill();ctx.font='12px system-ui';const confidence=axis.confidence??axis.probability;ctx.fillText(`Pred. ${axis.type} axis · conf. ${confidence.toFixed(2)}`,Math.max(12,Math.min(W-290,pivot[0]+12)),Math.max(75,Math.min(H-40,pivot[1]-14)));}}
